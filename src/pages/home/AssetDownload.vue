                  ...
                  <div class="thumb-wrap phone-aspect">
                    ...
                    <div class="card-actions">
                      <el-tooltip content="标注（弹窗）" placement="top">
                        <el-icon class="icon action" @click.stop="openRate(f)"><EditPen /></el-icon>
                      </el-tooltip>
                      <el-tooltip content="重命名" placement="top">
                        <el-icon class="icon action" @click.stop="openRenameDialog(f)"><Edit /></el-icon>
                      </el-tooltip>
                      <el-tooltip content="下载" placement="top">
                        <el-icon class="icon action" @click.stop="downloadOne(f)"><Download /></el-icon>
                      </el-tooltip>
                      <!-- 新增：删除 -->
                      <el-tooltip content="删除" placement="top">
                        <el-icon
                          class="icon action danger"
                          :class="{ disabled: deletingId===f.id }"
                          @click.stop="deletingId!==f.id && deleteOne(f)"
                        >
                          <DeleteIcon />
                        </el-icon>
                      </el-tooltip>
            ...
            <div class="rate-layout">
              ...
              >
                ...
                <div class="canvas-wrap" :style="canvasTransform" ref="canvasRef">
                  ...
                  <!-- 起草框 -->
                  <div v-if="hasBox" class="draw-box drafting" :style="boxStyle"></div>
                </div>

                <!-- 工具条（不随缩放） -->
                <div class="draw-toolbar" @mousedown.stop @click.stop>
                  <el-tag type="warning" size="small" class="pill-title">历史标注</el-tag><span class="legend history"></span>
                  <el-tag type="info" size="small" class="pill-title" style="margin-left:8px">新建标注</el-tag><span class="legend new"></span>
                  ...
                  <!-- <div class="zoom-ctrl">
                    ...
                    <el-button size="small" text @click="zoomReset">100%</el-button>
                  </div> -->

                  <!-- 画框按钮 -->
                  <el-button size="small" @click="saveCurrentBox" :disabled="!hasBox">保存当前框</el-button>
                  <el-button size="small" text type="danger" @click="clearBox" :disabled="!hasBox">清除当前框</el-button>
                  <!-- <el-tag size="small" effect="plain" class="kbd-tip">按住空格拖动画布</el-tag> -->
                </div>

                <div v-if="rateLoadingHistory" class="loading-mask">正在加载历史标注…</div>
              </div>

              <!-- 右：图片级分数 + 图片级标签 + 历史/新增勾选+备注 -->
              <div class="rate-form">
                <div class="section-title">
                  <span class="bar"></span>
                  <span class="text">图片分数</span>
                </div>
                <div class="anno-row">
                  <div class="field">
                    <el-slider v-model="imageScore" :min="0" :max="100" :step="1" show-input />
                  </div>
                </div>
                ...
                <div class="section-title">
                  <span class="bar"></span>
                  <span class="text">历史标注</span>
                </div>

                <div v-if="!historyBoxes.length && !rateLoadingHistory" class="no-new">无历史标注</div>

                <div v-else class="history-grid" v-loading="rateLoadingHistory">
                  <div
                    class="history-card"
                    v-for="(hb,i) in historyBoxes"
                    :key="'hf'+i"
                    :class="{active: i===activeHistoryIdx}"
                    @mouseenter="activeHistoryIdx=i"
                  >
                    <div class="card-head" @click="toggleCard(i)">
                      <div class="meta">
                        <el-checkbox v-model="hb.checked" @click.stop />
                        <span class="chip-id">#{{ i+1 }}</span>
                        <span class="chip-score">坐标：{{ hb.bbox.join(', ') }}</span>
                      </div>
                      <el-icon class="fold-icon" :class="{open:isOpen(i)}">
                        <svg viewBox="0 0 24 24" width="16" height="16"><path d="M8 10l4 4 4-4" fill="none" stroke="currentColor" stroke-width="2" /></svg>
                      </el-icon>
                    </div>

                    <transition name="slide-fade">
                      <div class="card-body" v-show="isOpen(i)">
                        <div class="card-row">
                          <div class="label sm">备注</div>
                          <div class="field">
                            <el-input v-model="hb.remark" placeholder="可填写备注（可选）" clearable />
                          </div>
                        </div>
                        <div class="tip-row">该框提交时会随“标签 / 分数”一并更新</div>
                      </div>
                    </transition>
                  </div>
                </div>

                <div class="section-title section-gap-lg">
                  <span class="bar"></span>
                  <span class="text">新增标注</span>
                  <span class="sub">（勾选的将提交）</span>
                </div>

                <div v-if="!newBoxes.length" class="no-new">暂无新建框，先拖拽并点击“保存当前框”</div>
                <div class="new-list" v-else>
                  <div class="one-new" v-for="(nb, idx) in newBoxes" :key="'form'+idx">
                    <div class="row">
                      <el-checkbox v-model="nb.checked">提交该框</el-checkbox>
                    </div>
                    <div class="row">
                      <div class="field">
                        <el-input v-model="nb.remark" type="textarea" :rows="2" maxlength="200" show-word-limit placeholder="可选：仅该框备注" />
                      </div>
                    </div>
                    <div class="row actions">
                      <el-button type="danger" plain size="small" class="btn-delete" @click="removeNew(idx)">
                        <el-icon class="btn-ic"><DeleteIcon /></el-icon>
                        删除该框
                      </el-button>
                    </div>
                    <el-divider />
                ...
                <div class="section-title section-gap-lg">
                  <span class="bar"></span>
                  <span class="text">快捷追加备注（统一）</span>
                </div>
                <div class="anno-row">
                  <div class="field">
                    <el-input v-model="rateForm.remark" placeholder="对所有勾选项追加统一备注（不覆盖每条备注）" />
                  </div>
                </div>
                <!-- 查看 / 缩放（放在右侧栏） -->
                <div class="section-title">
